...
```
L7  PRESENTATION    fairness purple + equity teal (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      bias-detection-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — shared mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: bias metrics, protected attributes, EU AI Act Article 10, fair-lending rules
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "biasdetectionof"
...
```bash
git clone https://github.com/CSOAI-ORG/biasdetectionof-hive
cd biasdetectionof-hive
...
`https://biasdetectionof.ai/.well-known/agent-card.json` — see `agent-card.json`.
...
£299/mo (cheapest single-MCP SaaS; fastest to monetise per DOMAINS.md).
